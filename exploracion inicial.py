...
print('Universidades distintas:', df['nombre_universidad'].nunique())
print('Carreras distintas:', df['nombre_carrera'].nunique())
...
print(pd.concat([null_counts.head(15), null_pct.head(15)], axis=1, keys=['n_nulos', '%_nulos']))

missing_id = df['id'].isna()
print(f'\nFilas sin "id" (ola de encuesta reducida): {missing_id.sum()}')
print(df.loc[missing_id, 'nombre_universidad'].value_counts())
...
separator('7. MOTIVOS DE POSIBLE ABANDONO (% que marca cada motivo)')
